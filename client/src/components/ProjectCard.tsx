            ...
            <div key= {gen.id} className="mb-4 break-inside-avoid">
                ...
                <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16': 'aspect-video'} relative overflow-hidden`}>
                    ...
                    {gen.generatedVideo && (
                        ...
                        className="absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition duration-500" onMouseEnter={(e) => e.currentTarget.play()}
                    ...
                    {!forCommunity && (
                        ...
                        className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-2">
                            ...
                            <div className="flex flex-col items-end w-32 text-sm">
                                <ul className={`tetx-xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/ 50 rounded-lg shadoW-md mt-2 py-1 z-10`}>

                                {gen.generatedImage && <a href="#" download className="flex gap-2 items-center px-2 py-2 hover:bg-black/10 cursor-pointer"> <ImageIcon size={14} />Download Image</a> }

                                {gen.generatedVideo && <a href="#" download className="flex gap-2 items-center px-2 py-2 hover:bg-black/10 cursor-pointer"> <PlaySquareIcon size={14} />Download Video</a> }
                ...
                <div className="p-4 ">
                    ...
                    <div className="flex items-start justify-between gap-4">                      
                        <div className="flex-1">
                            ...
                            {gen.updatedAt && (
                                <p className="text-sm text-gray-500 mt-1" >Updated: {new Date(gen.updatedAt).toLocaleString()}</p>
                            )}
                        </div>
                    ...
                    {gen.productDescription && (
                        <div className="mt-3">
                            ...
                            <div className="text-sm text-gray-300 bg-white/5 p-2 rounded-md break-word">{gen.productDescription}</div>
                        </div>
                    )}
